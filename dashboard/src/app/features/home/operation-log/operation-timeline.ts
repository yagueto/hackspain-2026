...
export class OperationTimeline {
  ...
  protected readonly visibleEvents = computed(() => this.log.visibleTimelineEvents(this.events()));
  ...
  constructor() {
    let previousIncidentId: string | undefined;
    let previousLastId: string | undefined;
    effect(() => {
      const incidentId = this.incidentId();
      const lastId = this.visibleEvents().at(-1)?.id;
      const appended =
        incidentId === 'INC-003' &&
      ...
      queueMicrotask(() => {
        ...
        this.element.nativeElement
          .querySelector<HTMLElement>('.timeline-event:last-child')
