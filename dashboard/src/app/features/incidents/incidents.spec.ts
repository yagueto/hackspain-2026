...
import { createOperationsMock, MOCK_INCIDENTS, MOCK_UNITS } from '../../core/data/operations.mock';
import { Operations } from '../../core/services/operations';
import { MOCK_INCIDENT_DETAILS } from './incidents.mock';
...
describe('Incidents workspace', () => {
  ...
  beforeEach(() => {
    ...
    operations.incidents.set(MOCK_INCIDENTS.slice(0, 4));
    operations.units.set(MOCK_UNITS.slice(0, 12));
    ...
    for (const incident of operations.incidents())
      store.applyUpdate({ incidentId: incident.id, details: MOCK_INCIDENT_DETAILS[incident.id] });
  });
