...
class WorldState:
    ...
    def _emit(self, type_: str, obj: BaseModel | dict[str, Any]) -> None:
        ...
        change = Change(type=type_, data=data)
    ...
    def available_resources(self, types: Iterable[str] | None = None) -> list[Resource]:
        ...
        return [
            ...
            if r.status == "available" and (wanted is None or r.type in wanted)
        ]
    ...
    def snapshot(self, recent: int = 50) -> WorldSnapshot:
        if self.incident is None or self.weather is None:
            raise RuntimeError("scenario not seeded")
        actions = sorted(self.actions.values(), key=lambda a: a.ts, reverse=True)[:recent]
        return WorldSnapshot(
            ...
            recent_events=list(self.events)[-recent:][::-1],
            recent_decisions=list(self.decisions)[-10:][::-1],
