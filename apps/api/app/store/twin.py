...
class TwinStore(Store):
    ...
    async def query(self, sql: str) -> list[dict[str, JsonValue]]:
        try:
            response = await self.client.post("/twin/sql", json={"sql": sql})
            response.raise_for_status()
            result = QueryResult.model_validate(response.json())
            if result.truncated:
                raise StoreError("Twin truncó el resultado; reduce el tamaño del lote")
            return result.rows
        except (httpx.HTTPError, ValidationError, ValueError) as exc:
            raise StoreError(f"Twin no disponible ({type(exc).__name__})") from exc

    async def inspect_schema(self) -> list[Table]:
        try:
            response = await self.client.get("/twin/schema")
            response.raise_for_status()
            return TypeAdapter(list[Table]).validate_python(response.json())
        except (httpx.HTTPError, ValueError) as exc:
            raise StoreError("no se pudo inspeccionar el esquema de Twin") from exc
